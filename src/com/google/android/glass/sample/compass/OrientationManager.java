...
public class OrientationManager {
  ...
  /**
   * The sensors used by the compass are mounted in the movable arm on Glass. Depending on how this
   * arm is rotated, it may produce a displacement ranging anywhere from 0 to about 12 degrees.
   * Since there is no way to know exactly how far the arm is rotated, we just split the difference.
   */
  ...
  /**
   * Initializes a new instance of {@code OrientationManager}, using the specified context to access
   * system services.
   */
  ...
  public void start() {
    if (!mTracking) {
      ...
      if (mLocationProvider != null) {
        mLocationManager.requestLocationUpdates(mLocationProvider, MILLIS_BETWEEN_LOCATIONS,
            METERS_BETWEEN_LOCATIONS, mLocationListener, Looper.getMainLooper());
      }
  ...
  /**
   * Gets a value indicating whether there is too much magnetic field interference for the compass
   * to be reliable.
   * 
  ...
  private void updateGeomagneticField() {
    mGeomagneticField =
        new GeomagneticField((float) mLocation.getLatitude(), (float) mLocation.getLongitude(),
